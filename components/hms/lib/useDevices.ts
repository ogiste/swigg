...
export const useDevices = (): useDevicesResult => {
  ...
  const peerRole = useHMSStore(selectLocalPeerRoleName);
  ...
  console.log(isAllowedToPublish);
  
  console.log('sdkSelectedDevices', sdkSelectedDevices);
  console.log('selectedDeviceIDs', selectedDeviceIDs );
  console.log('userRole', peerRole );

  if (!isAllowedToPublish.video) {
    console.log('hide video for this device')
    // TODO this was causing errors so I commented it out
  ...
  if (!isAllowedToPublish.audio) {
    ...
    console.log('hide audio for this device')
